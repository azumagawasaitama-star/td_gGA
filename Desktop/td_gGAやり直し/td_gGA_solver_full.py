"""
td_gGA_solver_full.py
Full TD-gGA solver: simultaneously evolves |Phi(t)> and n_ab(omega,t).
...
Equations of motion:
  ...
  [Eq.17]  dn_ab/dt  = -i*omega * [R_b R*_c n_ac - R*_a R_c n_cb]  (Lambda=0 gauge)
...
def prepare_full_params(ga_obj, U_final, N_freq=100):
    ...
    params = {
        ...
        'P_npz2imp': P_npz2imp,
        'Lmbda_npz': P_npz2imp.T @ ga_obj.Lmbda @ P_npz2imp,  # Λ in npz bath-orbital basis
    }
...
def compute_derivatives_full(t, y, params, ga_obj):
    ...
    v = np.einsum('fab,b->fa', n_omega, R_conj)   # (N_freq, nqspo)
    u = np.einsum('a,fab->fb', R_vec, n_omega)    # (N_freq, nqspo)
    dn_dt = -1j * omega_arr[:, None, None] * (
        np.einsum('fa,b->fab', v, R_vec) -
        np.einsum('a,fb->fab', R_conj, u)
    )
...
def run_full_simulation(ga_obj, U_final, t_max, dt, N_freq=100):
    ...
    Lmbda_npz = params['Lmbda_npz']
    for sc_iter in range(100):
        # Step2a: n(ω) from current R, Route B: n = f(ω·RR† + Λ)
        RopRt_cur = R_cur @ R_cur.T
        n0_iter = np.array([cr.calc_C(RopRt_cur * omega + Lmbda_npz, T=ga_obj.T)
        ...
        # Step2d: Lc = Lc_fast - Λ  (Route B)
        Lc_i = calc_Lmbdac_fast(Delta_i, D_i, R_cur,
                                 params['H_list'], params['H_list_arr'])
        Lc_i = Lc_i - np.real(Lmbda_npz)
        ...
        evals_i, evecs_i = np.linalg.eigh(H_emb_i)
        Phi_i = evecs_i[:, 0]
    ...
    ga_obj.D = D_i
    ga_obj.R = R_cur
    print(f"  E_0 = {evals_i[0]:.6f}  (gap = {evals_i[1]-evals_i[0]:.4f})")
    ...
    for k in range(len(sol.t)):
        ...
        Z_list.append(ga_obj.calc_Z(ga_obj.Lmbda, R_t_fg))
