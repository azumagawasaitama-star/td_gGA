...
def run_full_simulation(ga_obj, U_final, t_max, dt, N_freq=100):
    ...
    # Route A 純正初期化:
    #   ga_obj.Lmbda=0 (optimize_selfc_routeA で設定済み) なので
    #   n0(ω) = f(ωRR†) で初期化し、時間発展も同じゲージで行う。
    Lmbda_npz_init = np.zeros((nqspo, nqspo))   # Route A: Λ=0 固定
    ...
    for sc_iter in range(100):
        # Step2a: n(ω) = f(ω·RR†)  ← Route A 純正 (Λ=0)
